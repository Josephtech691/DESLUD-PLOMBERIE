...
const projects = [
  { id: 1, cat: 'installation', before: '🚽', after: '✨', title_fr: 'Installation salle de bain complète', title_en: 'Full bathroom installation', loc: 'Bastos, Yaoundé' },
  { id: 2, cat: 'reparation', before: '💧', after: '🔧', title_fr: 'Réparation fuite sous évier', title_en: 'Under-sink leak repair', loc: 'Nlongkak, Yaoundé' },
  { id: 3, cat: 'installation', before: '🚿', after: '✅', title_fr: 'Installation douche moderne', title_en: 'Modern shower installation', loc: 'Melen, Yaoundé' },
  { id: 4, cat: 'entretien', before: '⚙️', after: '👌', title_fr: 'Entretien réseau complet', title_en: 'Full network maintenance', loc: 'Mvog-Ada, Yaoundé' },
  { id: 5, cat: 'reparation', before: '🔴', after: '✅', title_fr: 'Remplacement tuyauterie rouillée', title_en: 'Rusty pipe replacement', loc: 'Biyem-Assi, Yaoundé' },
  { id: 6, cat: 'installation', before: '🚰', after: '💎', title_fr: 'Robinetterie haut de gamme', title_en: 'Premium faucet installation', loc: 'Elig-Essono, Yaoundé' },
];
          ...
          {filtered.map((p, i) => (
            <div key={p.id} onClick={() => setSelected(p)}
              ...
              <div className="relative flex h-48 overflow-hidden">
                <div className="flex-1 bg-gradient-to-br from-gray-200 to-gray-300 flex flex-col items-center justify-center">
                  <span className="text-5xl">{p.before}</span>
                  <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500 mt-2">{lang === 'fr' ? 'Avant' : 'Before'}</span>
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white rounded-full flex items-center justify-center shadow-md font-display font-black text-blue-deslud text-sm">→</div>
                <div className="flex-1 bg-gradient-to-br from-blue-deslud/10 to-cyan-deslud/20 flex flex-col items-center justify-center">
                  <span className="text-5xl">{p.after}</span>
                  <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-blue-deslud mt-2">{lang === 'fr' ? 'Après' : 'After'}</span>
                </div>
              </div>
              <div className="p-5">
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/80 z-[3000] flex items-center justify-center p-6" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl max-w-lg w-full p-8 relative" onClick={e => e.stopPropagation()}>
            <button onClick={() => setSelected(null)} className="absolute top-4 right-4 text-gray-400 hover:text-navy text-2xl font-bold">✕</button>
            <div className="flex gap-4 mb-6">
              <div className="flex-1 bg-gray-100 rounded-xl h-32 flex flex-col items-center justify-center">
                <span className="text-4xl">{selected.before}</span>
                <span className="text-xs font-bold uppercase tracking-wide text-gray-400 mt-2">{lang === 'fr' ? 'Avant' : 'Before'}</span>
              </div>
              <div className="flex-1 bg-blue-deslud/10 rounded-xl h-32 flex flex-col items-center justify-center">
                <span className="text-4xl">{selected.after}</span>
                <span className="text-xs font-bold uppercase tracking-wide text-blue-deslud mt-2">{lang === 'fr' ? 'Après' : 'After'}</span>
              </div>
            </div>
